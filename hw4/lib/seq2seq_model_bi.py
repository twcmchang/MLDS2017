  ...
  def __init__(self,
    ...
    with tf.variable_scope(self.scope_name):
      ...
      def seq2seq_f(encoder_inputs, decoder_inputs, feed_previous):
        return seq2seq_bi.google_mt_seq2seq(
            encoder_inputs, 
            decoder_inputs, 
            use_lstm,
...
#            softmax_loss_function=softmax_loss_function)
      ...
      # Training outputs and losses.
      self.outputs, self.losses, self.encoder_state = seq2seq_tf.model_with_buckets(
          self.encoder_inputs, self.decoder_inputs, targets,
          self.target_weights, buckets, 
          lambda x, y: seq2seq_f(x, y, tf.where(self.force_dec_input, False, True)), # True for feeding decoder input
          softmax_loss_function=softmax_loss_function
        )
      # If we use output projection, we need to project outputs for decoding.
      # if output_projection is not None:
      for b in xrange(len(buckets)):
        self.outputs[b] = [
          control_flow_ops.cond(
            self.en_output_proj, # 
            lambda: tf.matmul(output, output_projection[0]) + output_projection[1], # True
            lambda: output)
          for output in self.outputs[b]
        ]
      ...
      self.saver = tf.train.Saver(all_variables, max_to_keep=0)


  def step(self, session, encoder_inputs, decoder_inputs, target_weights,
           bucket_id, forward_only, force_dec_input=False, advantage=None, ScheduledSampling=False):
    ...
    # Scheduled sampling
    if ScheduledSampling:
      force_dec_input = np.random.rand() > 0.25
  ...
  def step_rf(self, args, session, encoder_inputs, decoder_inputs, target_weights,
    ...
    _, resp_txt = self.logits2tokens(encoder_inputs, rev_vocab, sent_max_length, True) # see current sentence
    print("[INPUT]:", resp_txt) # see current sentence
    ...
    while True:
      ...
      #print('encoder', np.shape(encoder_inputs))
      #print('decoder', np.shape(decoder_inputs))
      #print('state', np.shape(encoder_state))
      #print('loss', np.shape(step_loss))
      #print('logit', np.shape(output_logits)) # equal to decoder input shape
      # memorize inputs for reproducing curriculum with adjusted losses
      ep_encoder_inputs.append(encoder_inputs)
      ep_target_weights.append(target_weights)  # Why?
      ep_bucket_id.append(bucket_id)
      ep_step_loss.append(step_loss) # Why?
      enc_states_vec = np.reshape(encoder_state, (-1))  # Why?
      enc_states.append(enc_states_vec)
      
      # process response
      resp_tokens, resp_txt = self.logits2tokens(output_logits, rev_vocab, sent_max_length, False)
      # print('tokens', resp_tokens, len(resp_tokens)) # Have one "EOS"
      # if debug: print("[RESP]: (%.4f) %s" % (step_loss, resp_txt))

      # prepare for next dialogue
      # bucket_id = min([b for b in xrange(len(args.buckets)) if args.buckets[b][0] > len(resp_tokens)])
      bucket_id = [b for b in range(len(args.buckets)) if args.buckets[b][0] > len(resp_tokens)]
      if len(bucket_id) > 0:
        bucket_id = min(bucket_id)
      else:
        bucket_id = len(args.buckets)-1
      feed_data = {bucket_id: [(resp_tokens, [])]}
      encoder_inputs, decoder_inputs, target_weights = self.get_batch(feed_data, bucket_id) # zero decoder_inputs and target_weights
      ...
      if len(enc_states) < 2:
        ...
        r2 = float(sum(vec_a*vec_b)) / float(sum(abs(vec_a)*abs(vec_b)))
        r2 = -np.log(r2)
      
      # r3: Semantic Coherence
      # print(ep_encoder_inputs[-1])
      # print(encoder_inputs)
      # print(resp_tokens)	  
      r3 = -self.logProb(session, args.buckets, resp_tokens, ep_encoder_inputs[-1], deal_with_b = True)
    ...
    gradient_norms, step_loss, _ = self.step(session, init_inputs[0], init_inputs[1], init_inputs[2], init_inputs[3],
              forward_only=False, force_dec_input=False, advantage=advantage)
    
    return gradient_norms, step_loss, None



  # log(P(b|a)), the conditional likelyhood
  def logProb(self, session, buckets, tokens_a, tokens_b, deal_with_b):
    def softmax(x):
      return np.exp(x) / np.sum(np.exp(x), axis=0)

    # prepare for next dialogue
    if deal_with_b:
      tokens_b = [b[0] for b in tokens_b]
      tokens_b = list(reversed(tokens_b))
      if data_utils._PAD in tokens_b:
        pad = tokens_b.index(data_utils._PAD)
        tokens_b = tokens_b[:pad]
      if data_utils.EOS_ID in tokens_b:
        eos = tokens_b.index(data_utils.EOS_ID)
        tokens_b = tokens_b[:eos]
      tokens_b.append(data_utils.EOS_ID)
    # print('tokens_a',tokens_a, len(tokens_a))
    # print('tokens_b',tokens_b, len(tokens_b))
    bucket_id = [b for b in range(len(buckets)) if buckets[b][0] > len(tokens_a) and  buckets[b][1] > len(tokens_b)]
    if len(bucket_id) > 0:
      bucket_id = min(bucket_id)
    else:
      bucket_id = len(buckets)-1
      if data_utils.EOS_ID in tokens_a:
        eos = tokens_a.index(data_utils.EOS_ID)
        tokens_a = tokens_a[:eos]
      tokens_a = tokens_a[:(buckets[bucket_id][0]-1)]
      tokens_a.append(data_utils.EOS_ID)
      if data_utils.EOS_ID in tokens_b:
        eos = tokens_b.index(data_utils.EOS_ID)
        tokens_b = tokens_b[:eos]
      tokens_b = tokens_b[:(buckets[bucket_id][1]-1)]
      tokens_b.append(data_utils.EOS_ID)
      # print('fix_tokens_a',tokens_a, len(tokens_a))
      # print('fix_tokens_b',tokens_b, len(tokens_b))
  ...
  def logits2tokens(self, logits, rev_vocab, sent_max_length=None, reverse=False):
    ...
    tokens.append(data_utils.EOS_ID)
    txt.append(data_utils._EOS)
    return tokens, txt
